...
/**
 * Abstract Google login controller. To use it you need to extend this class
...
{
    /**
     * @var \PHPixie\Auth\Login\Google
     */
    protected $provider;

    /**
     * User ID of the google user
     * @var string
     */
    protected $user_id = '';
    ...
    public function before()
    {
        ...
        $this->provider = $this->pixie->auth->provider('google', $config);
        $this->default_return_url = $this->pixie->config->get("auth.{$config}.login.google.return_url", null);
        $this->return_url_key = "auth_{$config}_google_return";
    }
