...
class Category extends BaseModel {
     ...
     * @return null|Category
     */
    public function loadCategoryByAlias($categoryAlias){
        if (!$categoryAlias) {
            return null;
        }
        if ($categoryAlias == 'all') {
            $category = $this->pixie->orm->get('Category')->where('parent', 0)->find();
        } else {
            $category = $this->pixie->orm->get('Category')->where('hurl', $categoryAlias)->find();
        }
        if($category->loaded())
            return $category;
        return null;
    ...
    public function getPrintName()
    {
        if ($this->parent == 0) {
            ...
            return $params['root_category_name'] ?: 'All';
        } else {
            return $this->name;
        }
    }
