$(function() {
  ...
  var compare_table = $('div.page table.comparison');
  if(compare_table.length){
    compare_table.find('span.more').on('click touchend',function(){
      compare_table.toggleClass('bamboo carbon');
    });
  }
  ...
  $('#nav').mmenu({
      ...
      searchfield : {
        add: true,
        search: true,
        noResults: '<a href="/search?q=" class="search_link">No items found.<br />Advanced Search &#8594;</a>'
      },
  ...
  $('.product_slider').flexslider({
    ...
    before: function(slider) {
      var optionValue = $('.slides img:eq(' + slider.animatingTo + ')', $(slider[0])).attr('alt');
  ...
  $('#tag_filter').change(function() { window.location = $('#tag_filter').find('option').filter(':selected').val(); });
    ...
    $cf.on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
              var text = data.message ? data.message : 'Thanks for signing up!';
              $cf.prev('.message').html("<em>" + text + "</em>");
            }
        });
        e.preventDefault();
...
function onUpdateCart() {
    var cartPanel = $('#mm-m1-p0');
    $.ajax({
        url: '/cart/getSidebarCart',
        type: 'GET',
        dataType: 'json',
        timeout: 10000,
        success: function (res) {
            if (res.html) {
                cartPanel.find('li').filter('.cart_item, .mm-selected, .cart-buttons').remove();
                cartPanel.append(res.html);
            }
        }
    });
...
$(function () {
    ...
    // Prevent browser from jumping when clicking on tabs
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        if (href.substr(0, 1) == '#' && location.hash != href) {
            location.hash = href;
        }
        setTimeout(function () {document.documentElement.scrollTop = 0;}, 0);
    });
    ...
    (function () {
        ...
        $('.nav').each(function () {
            var nav = $(this);
            var tab = nav.find('a[data-toggle="tab"]').filter('[href="' + hash + '"]');
            if (tab.length) {
                tab.tab('show');
            }
        });
